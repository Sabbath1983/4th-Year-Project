...
class GameSceneTwo: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        //Score Label
        GameSceneTwo.scoreLabel = SKLabelNode(text: "Score: 0")
        GameSceneTwo.scoreLabel.position = CGPoint(x: -300, y: 525)
        GameSceneTwo.scoreLabel.fontName = "Symbol"
        GameSceneTwo.scoreLabel.fontSize = 28
        GameSceneTwo.scoreLabel.fontColor = UIColor.white
        GameSceneTwo.score = GameSceneOne.score
        addChild(GameSceneTwo.scoreLabel)
        
        //Lives Label
        livesLabel = SKLabelNode(text: "Lives: 0")
        livesLabel.position = CGPoint(x: 300, y: 525)
        livesLabel.fontName = "Symbol"
        livesLabel.fontSize = 28
        livesLabel.fontColor = UIColor.white
        numLives = GameSceneOne.numLives
        addChild(livesLabel)
        ...
        bossTimer = Timer.scheduledTimer(timeInterval: 90.0, target: self, selector: #selector(addBoss), userInfo: nil, repeats: false)
        ...
        run(SKAction.repeatForever(
          SKAction.sequence([SKAction.run() { [weak self] in
                            ...
                            SKAction.wait(forDuration: 3.0)])), withKey:"enemy1")
        ...
        run(SKAction.repeatForever(
          SKAction.sequence([SKAction.run() { [weak self] in
                            ...
                            SKAction.wait(forDuration: 4.0)])), withKey:"enemy2")
        ...
        run(SKAction.repeatForever(
          SKAction.sequence([SKAction.run() { [weak self] in
                            ...
                            SKAction.wait(forDuration: 5.0)])), withKey:"enemy3")
    ...
    func addEnemy1() {
        ...
        //Enemy1 Image
        enemy1 = .init(imageNamed: "Enemy7.png")
    ...
    func addEnemy2() {
        //Enemy2 Image
        enemy2 = .init(imageNamed: "Enemy8.png")
        ...
        let delay = SKAction.wait(forDuration: 17)
        let actionMove = SKAction.move(to: endPoint, duration: 1.5)
        let actionRemove = SKAction.removeFromParent()
    ...
    func addEnemy3() {
        ...
        //Enemy3 Image
        enemy3 = .init(imageNamed: "Enemy9.png")
        ...
        //Delay enemies
        let delay = SKAction.wait(forDuration: 29)

        //Move enemies to next postions randomly on the x axis while moving down the y axis
        let actionMoveOne = SKAction.move(to: CGPoint(x: position, y: 500), duration: 0.5)
        position = CGFloat(randomEnemy3Position.nextInt())
        let actionMoveTwo = SKAction.move(to: CGPoint(x: position, y: 200), duration: 0.5)
        position = CGFloat(randomEnemy3Position.nextInt())
        let actionMoveThree = SKAction.move(to: CGPoint(x: position, y: -100), duration: 0.5)
        position = CGFloat(randomEnemy3Position.nextInt())
        let actionMoveFour = SKAction.move(to: CGPoint(x: player.position.x, y: -400), duration: 0.5)
        position = CGFloat(randomEnemy3Position.nextInt())
        let actionMoveFive = SKAction.move(to: CGPoint(x: position, y: -600), duration: 0.5)
    ...
    func addBoss() {
        ...
        //Boss Image
        boss = .init(imageNamed: "BossC.png")
...
@objc func addBossLaser() {
    ...
    func playerLaserHitBoss (laserNode:SKSpriteNode, bossNode:SKSpriteNode) {
        ...
        if bossHealth > 1 {
            ...
            //Add score
            GameSceneTwo.score += 25
            
            //Condition for achievement
            //GameSceneTwo.bossIsDead = true
            ...
            let endGameTransistion = SKAction.run() {
                ...
                complateLevel.score = GameSceneTwo.score
    ...
    func playerHitAstroid(playerNode:SKSpriteNode, astroidNode:SKSpriteNode) {
        ...
        if numLives > 0 && invincible == false {
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if (GameSceneTwo.bossIsDead == true) {
            //reportAllAchievementsForGameState(bossIsDead: true)
            reportScoreToGameCenter(score: Int64(GameSceneTwo.score))
        }
        else if (GameSceneTwo.bossIsDead == false) {
